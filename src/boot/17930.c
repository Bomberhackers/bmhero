...
#define DEFAULT_LIGHTS_CONF                                                                               \
    gdSPDefLights2(0x32, 0x32, 0x32, 0, 0, 0, 0xA, 0x2A, 0x78, /* Light 1 */ 0xC8, 0xC8, 0xC8, 0xA, 0x2A, \
                   0x78) /* Light 2 */

//.data
Lights2 sDefaultLightSettings = DEFAULT_LIGHTS_CONF;
Lights2 gLightingSettings = DEFAULT_LIGHTS_CONF;
...
s16 func_80017460(s32 obj_idx) {
    ...
    obj = &gObjects[obj_idx];
    distX = gView.at.x - obj->Pos.x;
...
UNUSED void Unused_DebugFunction(void) {
    u16 persp;
    UNUSED f32 sqrt;
...
s32 Get_Level_LightsType(void) {
    Lights2* lights = gLevelInfo[gCurrentLevel]->unk20;

    if (lights) {
        gLightingSettings = *lights;

        if ((gCurrentLevel == MAP_FREEZE_LAKE) || (gCurrentLevel == MAP_SNOWLAND)) {
            return LEVEL_LIGHTS_2;
        } else {
            return LEVEL_LIGHTS_1;
        }
    }
    return LEVEL_LIGHTS_0;
}
...
void Reset_LightSettings(void) {
    gLightingSettings = sDefaultLightSettings;
}
...
void func_800183E8(s32 r, s32 g, s32 b, s32 a, s32 arg4, s32 arg5) {
    ...
    gDPSetRenderMode(gMasterDisplayList++,
                     AA_EN | Z_CMP | Z_UPD | CVG_DST_CLAMP | ZMODE_OPA | CVG_X_ALPHA | ALPHA_CVG_SEL | FORCE_BL |
                         G_RM_FOG_SHADE_A,
                     AA_EN | Z_CMP | Z_UPD | CVG_DST_CLAMP | ZMODE_OPA | CVG_X_ALPHA | ALPHA_CVG_SEL | FORCE_BL |
...
/**
 * Initialize a specific game object given the index
 */
...
void func_8001A738(s32 objIdx, s32 arg1) {
    s32 sp1C;

    if ((sp1C = gObjects[objIdx].Unk140[arg1]) != -1) {
        func_8001A5F8(sp1C);
        func_80019BC8(sp1C);
        gObjects[objIdx].Unk140[arg1] = -1;
    }
}
...
void func_8001A88C(s32 arg0, s32 arg1) {
    ...
    for (i = 0; i < 4; i++) {
        func_8001A738(arg0, i);
    }

    for (i = 0; i < 4; i++) {
        func_8001A7E8(arg0, i);
    }
...
void func_8001EB68(s32 id, void* rom_start, void* rom_end) {
    DecompressFile((u32) id, (u32) rom_start, (u32) rom_end);
    if (Get_Level_LightsType() == LEVEL_LIGHTS_1) {
        func_800180C4((u32) gFileArray[id].ptr, Get_DecompressHeap());
    }
}
