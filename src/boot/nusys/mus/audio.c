...
after:;
    ...
    osCreateThread(&__am.thread, amc->threadID, __amMain, NULL, (void*) (audioStack + AUDIO_STACKSIZE / sizeof(u64)),
                   amc->pri);
...
static s32 __amHandleFrameMsg(AudioInfo* info, AudioInfo* lastInfo) {
    ...
    info->frameSamples = 16 + ((frameSize - samplesLeft + EXTRA_SAMPLES) &
                               ~0xf); // need the extra parens to match.. why, its not there in the demo code
...
static s32 __amDMA(s32 addr, s32 len, void* state) {
    ...
    osPiStartDma(&audio_IO_mess_buf[nextDMA++], OS_MESG_PRI_NORMAL, OS_READ, (u32) addr, foundBuffer, audio_dma_length,
                 &audDMAMessageQ);
