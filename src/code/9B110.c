...
void func_800ACB2C(void)
{
    ...
    sp24->unkD8 = sinf((f32)((f64)sp24->unkA8 * 0.0174532925199432955)) * 32.0f;
...
void func_800AF148(void)
{
    ...
    if (sp28->unkE6[0] != -1)
    {
        ...
        sp24 *= sp20;
        sp28->Pos.x = sp2C->Pos.x + (sinf((f32)((f64)sp2C->unk3C * 0.0174532925199432955)) * sp24);
        sp28->Pos.z = sp2C->Pos.z + (cosf((f32)((f64)sp2C->unk3C * 0.0174532925199432955)) * sp24);
...
void func_800B0C1C(void)
{
    ...
    sp26 = func_80027464(2, &D_801139AC, (f32)D_80165100->unk2, (f32)D_80165100->unk4, (f32)D_80165100->unk6, (f32)D_80165100->unk8);
    if (sp26 != -1)
    {
        gObjects[sp26].unkA4 = 1;
...
#define NORMALIZE_ANGLE(angle, offset) \
    (((angle) + (offset)) > 180.0f ? (((angle) + (offset)) >= 360.0f ? ((angle) + (offset)) - 360.0f : (angle) + (offset)) : (((angle) + (offset)) < 0.0f ? ((angle) + (360.0f + (offset))) : (angle) + (offset)))
...
void func_800B36D4(void)
{
    ...
    if (sp30->unkE6[0] != -1)
    {
        ...
        if (((sp34->Vel.x < 0.0f) ? -sp34->Vel.x : sp34->Vel.x) >= ((sp34->Vel.z < 0.0f) ? -sp34->Vel.z : sp34->Vel.z))
        {
            sp30->Rot.x =
                ((sp30->Rot.x + sp34->Vel.x) > 180.0f)
                    ? (((sp30->Rot.x + sp34->Vel.x) >= 360.0f) ? ((sp30->Rot.x + sp34->Vel.x) - 360.0f) : (sp30->Rot.x + sp34->Vel.x))
                    : (((sp30->Rot.x + sp34->Vel.x) < 0.0f) ? (sp30->Rot.x + 360.0f + sp34->Vel.x) : (sp30->Rot.x + sp34->Vel.x));
        }
        else
        {
            sp30->Rot.x =
                ((sp30->Rot.x + sp34->Vel.z) > 180.0f)
                    ? (((sp30->Rot.x + sp34->Vel.z) >= 360.0f) ? ((sp30->Rot.x + sp34->Vel.z) - 360.0f) : (sp30->Rot.x + sp34->Vel.z))
                    : (((sp30->Rot.x + sp34->Vel.z) < 0.0f) ? (sp30->Rot.x + 360.0f + sp34->Vel.z) : (sp30->Rot.x + sp34->Vel.z));
        }
    }
    else
    {
...
void func_800B3A80(void)
{
    ...
    sp26 = func_80027464(1, &D_801139D0, (f32)D_80165100->unk2, (f32)D_80165100->unk4, (f32)D_80165100->unk6, (f32)D_80165100->unk8);
    if (sp26 != -1)
    {
        gObjects[sp26].unkA4 = 1;
        gObjects[sp26].unk132 = 0;
...
int func_800B5368(struct ObjectStruct *arg0, s32 arg1, s32 arg2)
{
    ...
    if (sp2A != -1)
    {
        ...
        sp2C->Vel.x = 140.0f * sinf((f32)((f64)sp24 * 0.0174532925199432955));
        sp2C->Vel.z = 140.0f * cosf((f32)((f64)sp24 * 0.0174532925199432955));
